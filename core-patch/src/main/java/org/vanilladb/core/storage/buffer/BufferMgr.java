...
public class BufferMgr implements TransactionLifecycleListener {
	...
	private final Condition bufferAvailable = poolLock.newCondition();
			...
			try {
				...
				if (buff == null) {
					...
					while (buff == null && !waitingTooLong(timestamp)) {
						bufferAvailable.await(MAX_TIME, TimeUnit.MILLISECONDS);
	...
	private void unpinAll(Transaction tx) {
		...
		try{
			...
			pinningBuffers.clear();
			//buffersToFlush.clear();
